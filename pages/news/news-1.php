<section id="main" class="main" role="main">
    <div class="layoutBoundary">
        <div id="content" class="content">
            <div class="contentHeader messageGroupContentHeader wbbThread" data-thread-id="8087" data-is-link="0">
                <div class="contentHeaderIcon">
                </div>
                <div class="contentHeaderTitle">
                    <h1 class="contentTitle">[24/10/2022] Balance changes</h1>
                    <ul class="inlineList contentHeaderMetaData">
                        <li>
                            <span class="icon icon16"><i class="fa fa-user" aria-hidden="true"></i></span>
                            <span class="userLink" style="color: #cb3a35;"><?php echo $site['name']; ?></span>
                        </li>
                        <li>
                            <span class="icon icon16"><i class="fa fa-clock-o" aria-hidden="true"></i></span>
                            <time datetime="2022-10-24T01:43:00+02:00" class="datetime" data-timestamp="1666568580" data-date="Oct 24th 2022" data-time="1:43 am" data-offset="7200">Monday, 1:43 am</time></a>
                        </li>
                    </ul>
                </div>
            </div>
            <div class="section">
                <ul class="wbbThreadPostList messageList jsClipboardContainer" data-type="com.woltlab.wbb.post">
                    <li id="post29919" class="anchorFixedHeader messageGroupStarter">
                        <article class="wbbPost message messageSidebarOrientationLeft jsClipboardObject jsMessage userOnlineGroupMarking25">
                            <meta itemprop="dateCreated" content="2022-10-24T01:43:00+02:00">
                            <aside class="messageSidebar member" itemprop="author" itemscope itemtype="http://schema.org/Person">
                                <div class="messageAuthor">
                                    <div class="userAvatar">
                                        <img src="/assets/img/logo.png" style="width: 80px; height: 80px" alt="" class="userAvatarImage">
                                    </div>
                                    <div class="messageAuthorContainer">
                                        <span class="username userLink" style="color: #d63b37;" itemprop="name"><?php echo $site['name']; ?></span>
                                    </div>
                                    <div class="userTitle">
                                        <span class="badge-new userTitlebadge-new Apolys"><?php echo $site['name']; ?></span>
                                    </div>
                                </div>
                            </aside>
                            <div class="messageContent">
                                <div class="messageHeader">
                                    <div class="messageHeaderBox">
                                        <ul class="messageHeaderMetaData">
                                            <a class="permalink messagePublicationTime"><time datetime="2022-10-24T01:43:00+02:00" class="datetime" data-timestamp="1666568580" data-date="Oct 24th 2022" data-time="1:43 am" data-offset="7200">Monday,
                                                        1:43 am</time></a></li>
                                        </ul>
                                    </div>
                                </div>
                                <div class="messageBody">
                                    <div class="messageText" itemprop="text">
                                        <p class="text-center">
                                            <img src="../assets/img/patch.png" alt="patch.png" class="jsResizeImage"><br>
                                        </p>
                                        <p><br></p>
                                        <p><br></p>
                                        <p><strong><span style="font-size: 14pt;"><img src="/assets/img/changes.png" alt="changes.png" class="jsResizeImage"></span></strong>
                                        </p>
                                        <p><br></p>
                                        <p>— <img src="/assets/img/items/4161.png" alt="4161.png" class="jsResizeImage"> Lion Loa Sword (Swordman C55 main weapon) changes:</p>
                                        <p>Critical chance:
                                            <span style="color:#FF0000;"><del>14%</del></span> 
                                            <i class="fa fa-long-arrow-right" aria-hidden="true"></i>
                                             <strong><span style="color:#008000;">16%</span></strong>
                                        </p>
                                        <p>Hit rate:
                                            <span style="color:rgb(255,0,0);"><del>559</del></span> 
                                            <i class="fa fa-long-arrow-right" aria-hidden="true"></i>
                                            </fa-icon>
                                            <span style="color:#008000;"><strong> 609</strong></span>
                                        </p>
                                        <p><br></p>
                                        <p>— <img src="/assets/img/items/4164.png" alt="4164.png" class="jsResizeImage"> Eagle Loa Bow (Archer C55 main weapon) changes:</p>
                                        <p>Damage:
                                            <span style="color:rgb(255,0,0);"><del>882~922</del></span> 
                                            <i class="fa fa-long-arrow-right" aria-hidden="true"></i>
                                            
                                             <strong><span style="color:#008000;">1400~1450</span></strong>
                                        </p>
                                        <p><br></p>
                                        <p>— <img src="/assets/img/items/4176.png" alt="4176.png" class="jsResizeImage"> Bat Loa Dagger (Archer C55 secondary weapon) changes:</p>
                                        <p>Damage:
                                            <span style="color:rgb(255,0,0);"><del>647~687</del></span> 
                                            <i class="fa fa-long-arrow-right" aria-hidden="true"></i>
                                             <span style="color:#008000;"><strong>1058~1098</strong></span>
                                        </p>
                                        <p>Critical damage:
                                            <span style="color:rgb(255,0,0);"><del>350%</del></span> 
                                            <i class="fa fa-long-arrow-right" aria-hidden="true"></i>
                                            <strong><span style="color:#008000;"> 320%</span></strong>
                                        </p>
                                        <p>Increases damage with a
                                            probability of 10% by
                                            <span style="color:rgb(255,0,0);"><del>75%</del></span> 
                                            <i class="fa fa-long-arrow-right" aria-hidden="true"></i>
                                             <span style="color:#008000;"><strong>40%</strong></span>
                                        </p>
                                        <p><br></p>
                                        <p>— <img src="/assets/img/items/4188.png" alt="4188.png" class="jsResizeImage"> Eagle Loa Leather Armour (Archer C55 armor) changes:</p>
                                        <p>Melee Defence:
                                            <span style="color:rgb(255,0,0);"><del>627</del></span> 
                                            <i class="fa fa-long-arrow-right" aria-hidden="true"></i>
                                             <strong><span style="color:#008000;">527</span></strong>
                                        </p>
                                        <p>Ranged Defence:
                                            <span style="color:rgb(255,0,0);"><del>697</del></span> 
                                            <i class="fa fa-long-arrow-right" aria-hidden="true"></i>
                                             <span style="color:#008000;"><strong>597</strong></span>
                                        </p>
                                        <p>Magic Defence:
                                            <span style="color:rgb(255,0,0);"><del>694</del></span> 
                                            <i class="fa fa-long-arrow-right" aria-hidden="true"></i>
                                             <strong><span style="color:#008000;">594</span></strong>
                                        </p>
                                        <p><br></p>
                                        <p>— <img src="/assets/img/items/4170.png" alt="4170.png" class="jsResizeImage"> Bear Loa Gauntlets (Martial Artist C55 main weapon) changes:</p>
                                        <p>Critical chance:
                                            <span style="color:rgb(255,0,0);"><del>22%</del></span> 
                                            <i class="fa fa-long-arrow-right" aria-hidden="true"></i>
                                             <strong><span style="color:#008000;">20%</span></strong><br>
                                        </p>
                                        <p><br></p>
                                        <p>— Now enemy's percentage defense reduction in PvP mode shell weapon effect will work as follows:</p>
                                        <ul>
                                            <li>
                                                <strong>When hits are soft damage (<em><strong>increased damages &amp; increased damages + critical hits</strong></em>)
                                                <i class="fa fa-long-arrow-right" aria-hidden="true"></i>                                                </strong>
                                                The reduction will be only shell enemy's percentage defense reduction effects.
                                            </li>
                                            <li>
                                                <strong>When hits are normal damage (<em>includes critical hits</em>) <i class="fa fa-long-arrow-right" aria-hidden="true"></i> </strong>
                                                The reduction will be total enemy's defense.
                                            </li>
                                        </ul>
                                        <p><br></p>
                                        <p>— Now all percentage <span style="color:#EE82EE;">damage reflection</span> effects can reflect a maximum of 7500 damage.</p>
                                        <p><em><span style="font-size: 10pt;"><span style="color:#FF0000;">Note: </span></span></em><em><span style="font-size: 10pt;">In</span></em><em><span style="font-size: 10pt;"> the case of stacked damage reflection the maximum of 7500 damage will be applied to each stacked hit and not to the final stacked reflection damage.</span></em>
                                        </p>
                                        <p><br></p>
                                        <p>— Now conversion received damage by reducing MP effect will only apply to 50% of the total MP consumption reduction you have.</p>
                                        <p><br></p>
                                        <p>— Added a maximum value of 80% to the accumulation of MP consumption reduction.</p>
                                        <p><br></p>
                                        <p>— Now it isn't allow to use the `[<img src="/assets/img/items/31271.png" alt="31271.png" class="jsResizeImage"> Echo Power] effect twice in a row with the same skill in less than 1 minute.</p>
                                        <p><br></p>
                                        <p>— Reduced the cooldown reduction from 90% to 75% in `[<img src="/assets/img/items/31271.png" alt="31271.png" class="jsResizeImage"> Echo Power] rune.</p>
                                        <p><br></p>
                                        <p>— The time to do forced movement effects in skills will now take into account the distance between the damager and the target.<br></p>
                                        <p><br></p>
                                        <p>— Increased the probability of inflicting the
                                            [<img width="20" src="/assets/img/items/50726.png" alt="50726.png" class="jsResizeImage"> Jötunheim Dragon's Breath] effect from 3% to 5% in
                                            <img src="/assets/img/items/50703.png" alt="50703.png" class="jsResizeImage">Jötunheim Dragon's Costume Wings.
                                        </p>
                                        <p><br></p>
                                        <p>— Increased duration of
                                            [<img src="/assets/img/items/10340.png" alt="10340.png" class="jsResizeImage"> Hard as Ice] from 3 to 5 seconds in
                                            <img src="/assets/img/items/4283.png" alt="4283.png" class="jsResizeImage"> Ice Witch Costume Wings
                                        </p>
                                        <p><br></p>
                                        <p>— <img src="/assets/img/items/50735.png" alt="50735.png" class="jsResizeImage"> Jötunheim Blessing of the 4 classes changes:</p>
                                        <p>· <span style="color:#FF0000;"><strong>Removed:</strong> Damage against Glacernon bosses increases by 10%</span>
                                        </p>
                                        <p>· <span style="color:#008000;"><strong>Added:</strong> </span><span style="color:#FF8C00;">Damage against bosses increases by 8%</span><br></p>
                                        <p><br></p>
                                        <p>— <img src="/assets/img/items/50735.png" alt="50735.png" class="jsResizeImage"> Jötunheim Blessing Magician changes:</p>
                                        <p>· <span style="color:#FF8C00;">Attack power increases by</span> <span style="color:#FF0000;"><del>8%</del></span> 
                                            <i class="fa fa-long-arrow-right" aria-hidden="true"></i>
                                             <strong><span style="color:#008000;">10%</span></strong>
                                        </p>
                                        <p>· <span style="color:#FF8C00;">Reduces the enemy's elemental resistances by</span> <span style="color:#FF0000;"><del>5</del></span> 
                                            <i class="fa fa-long-arrow-right" aria-hidden="true"></i>
                                             <span style="color:#008000;"><strong>15</strong></span>
                                        </p>
                                        <p><br></p>
                                        <p><br></p>
                                        <p><br></p>
                                        <p><strong><span style="font-size: 24pt;">Specialist changes</span></strong>
                                        </p>
                                        <p><br></p>
                                        <div class="spoilerBox jsSpoilerBox">
                                            <div class="jsOnly spoilerBoxHeader">
                                                <img src="/assets/img/items/32040.png ">
                                                <a class="news-button small jsSpoilerToggle" data-has-custom-label="true">
                                                    •
                                                    SWORDMAN</a>
                                            </div>
                                            <div class="spoilerBoxContent" style="display: none;">
                                                <p><strong><img width="50" src="/assets/img/items/32502.png" alt="32502.png" class="jsResizeImage"> <span style="font-size: 14pt;">Warrior</span></strong><br></p>
                                                <ul>
                                                    <li>
                                                        <strong><img src="/assets/img/items/5813.png" alt="5813.png" class="jsResizeImage"> Iron Skin</strong>
                                                        <ul>
                                                            <li><span style="color:#FF0000;"><strong>Removed: </strong>There is a 100% chance of causing [<img width="20" src="/assets/img/items/50667.png" alt="50667.png" class="jsResizeImage"> Total Absorption]</span>
                                                            </li>
                                                            <li>
                                                                <strong><img width="20" src="/assets/img/items/5813.png" alt="5813.png" class="jsResizeImage"> Iron Skin (Buff)</strong>
                                                                <ul>
                                                                    <li>Duration:
                                                                        <span style="color:rgb(255,0,0);"><del>30</del></span> 
                                                                        <i class="fa fa-long-arrow-right" aria-hidden="true"></i>
                                                                         <span style="color:rgb(0,128,0);"><strong>600 seconds</strong></span>
                                                                    </li>
                                                                    <li><span style="color:#FF0000;"><strong><strong>Removed: </strong></strong>Decrease cooldown of skills by 15% max. 90)</span>
                                                                    </li>
                                                                </ul>
                                                            </li>
                                                        </ul>
                                                    </li>
                                                    <li>
                                                        <strong><img src="/assets/img/items/5814.png" alt="5814.png" class="jsResizeImage"> Whirlwind</strong><br>
                                                        <ul>
                                                            <li>
                                                                <strong>Cool time:</strong> <span style="color:#FF0000;"><del>18</del></span> 
                                                                <i class="fa fa-long-arrow-right" aria-hidden="true"></i>
                                                                 <strong><span style="color:#008000;">15 seconds</span></strong>
                                                            </li>
                                                        </ul>
                                                    </li>
                                                    <li>
                                                        <strong><img src="/assets/img/items/5815.png" alt="5815.png" class="jsResizeImage"> Provoke</strong>
                                                        <ul>
                                                            <li><span style="color:#FF0000;"><strong>Removed</strong>: There is a 100% chance of causing [<img src="/assets/img/items/10155.png" alt="10155.png" class="jsResizeImage"> Paralisys]</span>
                                                            </li>
                                                        </ul>
                                                        <ul>
                                                            <li>
                                                                <span style="color:#008000;"><strong>Added:</strong> </span><span style="color:#FF8C00;">There is a 100% chance of causing [<img src="/assets/img/items/10305.png" alt="10305.png" class="jsResizeImage"> Provocation]</span>
                                                            </li>
                                                        </ul>
                                                        <ul>
                                                            <li>
                                                                <strong><span style="color:#008000;"><strong>Added</strong>:
                                                                    </span>
                                                                </strong><span style="color:#FF8C00;"><strong></strong> There is a 100% chance of causing [<img width="20" src="/assets/img/items/50805.png" alt="50805.png" class="jsResizeImage"> Paralysis]<br></span>
                                                                <ul>
                                                                    <li>
                                                                        <img width="20" src="/assets/img/items/50805.png" alt="50805.png" class="jsResizeImage"> Paralysis
                                                                        <ul>
                                                                            <li>Debuff Level 3</li>
                                                                            <li>Duration: 2 seconds</li>
                                                                            <li><span style="color:#FF8C00;">Movement impossible</span>
                                                                            </li>
                                                                            <li><span style="color:#FF8C00;">No attack possible</span>
                                                                            </li>
                                                                        </ul>
                                                                    </li>
                                                                </ul>
                                                            </li>
                                                        </ul>
                                                    </li>
                                                    <li>
                                                        <strong><img src="/assets/img/items/5817.png" alt="5817.png" class="jsResizeImage"> Intimidate</strong>
                                                        <ul>
                                                            <li>
                                                                <strong><strong>Cool time:</strong> </strong><span style="color:rgb(255,0,0);"><del>30</del></span> 
                                                                <i class="fa fa-long-arrow-right" aria-hidden="true"></i>
                                                                <span style="color:rgb(0,128,0);"> </span><strong><span style="color:#008000;">20 seconds</span></strong>
                                                            </li>
                                                            <li>
                                                                <strong>Target: </strong>Around Enemy in
                                                                <span style="color:#FF0000;"><del>5</del></span>  <i class="fa fa-long-arrow-right" aria-hidden="true"></i>
                                                                 <span style="color:#008000;"><strong>8</strong></span> cell
                                                            </li>
                                                        </ul>
                                                        <ul>
                                                            <li>
                                                                <span style="color:#008000;"><strong>Added:</strong> </span><span style="color:rgb(255,140,0);">There is a 100% chance of causing [<img width="20" src="/assets/img/items/50667.png" alt="50667.png" class="jsResizeImage"> Total Absorption]</span>
                                                            </li>
                                                        </ul>
                                                    </li>
                                                    <li>
                                                        <strong><img src="/assets/img/items/50373.png" alt="50373.png" class="jsResizeImage"> Continuous Load</strong><br>
                                                        <ul>
                                                            <li>
                                                                <strong>Cool time:</strong><strong></strong> <span style="color:rgb(255,0,0);"><del>15</del></span> 
                                                                <i class="fa fa-long-arrow-right" aria-hidden="true"></i>
                                                                 <strong><span style="color:#008000;">12 seconds</span></strong>
                                                            </li>
                                                        </ul>
                                                    </li>
                                                    <li>
                                                        <strong><img src="/assets/img/items/5820.png" alt="5820.png" class="jsResizeImage"> Earth Shock</strong><br>
                                                        <ul>
                                                            <li>
                                                                <strong>Cool time: </strong><span style="color:#FF0000;"><del>40</del></span><strong> </strong>
                                                                <i class="fa fa-long-arrow-right" aria-hidden="true"></i>
                                                                <strong> <span style="color:#008000;">35 seconds</span></strong>
                                                            </li>
                                                        </ul>
                                                    </li>
                                                    <li>
                                                        <strong><img src="/assets/img/items/5821.png" alt="5821.png" class="jsResizeImage"> Rising Dragon</strong><br>
                                                        <ul>
                                                            <li>
                                                                <strong><strong>Cool time:</strong> </strong><span style="color:rgb(255,0,0);"><del>120</del></span> 
                                                                <i class="fa fa-long-arrow-right" aria-hidden="true"></i>
                                                                 <strong><span style="color:#008000;">100 seconds</span></strong>
                                                            </li>
                                                        </ul>
                                                    </li>
                                                </ul>
                                                <p><br></p>
                                            </div>
                                        </div>
                                        <div class="spoilerBox jsSpoilerBox">
                                            <div class="jsOnly spoilerBoxHeader">
                                                <img src="/assets/img/items/32080.png "> <a class="news-button small jsSpoilerToggle" data-has-custom-label="true"> • ARCHER</a>
                                            </div>
                                            <div class="spoilerBoxContent" style="display: none;">
                                                <p><strong><span style="font-size: 14pt;"><img src="" alt="unknown.png" class="jsResizeImage"> Ranger</span></strong>
                                                </p>
                                                <ul>
                                                    <li>
                                                        <strong><img src="" width="20" alt="unknown.png" class="jsResizeImage"> Hawk Eye (Buff)</strong>
                                                        <ul>
                                                            <li>
                                                                <span style="color:#FF8C00;">There is a </span><span style="color:rgb(255,0,0);"><del>95%</del></span><span style="color:rgb(255,140,0);"> </span>
                                                                <i class="fa fa-long-arrow-right" aria-hidden="true"></i>
                                                                <span style="color:rgb(255,140,0);"> </span><span style="color:rgb(0,128,0);"><strong>80%</strong></span><span style="color:rgb(255,140,0);"> chance that every attack hits</span>
                                                            </li>
                                                        </ul>
                                                    </li>
                                                    <li>
                                                        <strong><img src="/assets/img/items/5837.png" alt="5837.png" class="jsResizeImage"> Flashbang</strong>
                                                        <p><br></p>
                                                        <ul>
                                                            <li>
                                                                <strong>Target: </strong>Around Enemy in
                                                                <span style="color:#FF0000;"><del>2</del></span> <i class="fa fa-long-arrow-right" aria-hidden="true"></i>
                                                                 <strong><span style="color:#008000;">3</span></strong> cell
                                                            </li>
                                                        </ul>
                                                    </li>
                                                    <li>
                                                        <strong><img src="/assets/img/items/5838.png" alt="5838.png" class="jsResizeImage"> Triple Arrow</strong>
                                                        <ul>
                                                            <li>
                                                                <strong>Target: </strong><span style="color:#FF0000;"><del>Special Area</del></span> 
                                                                <i class="fa fa-long-arrow-right" aria-hidden="true"></i>
                                                                 <span style="color:#008000;"><strong>Around Enemy in 2 cell</strong></span>
                                                            </li>
                                                        </ul>
                                                    </li>
                                                    <li>
                                                        <strong><img src="/assets/img/items/5842.png" width="20" alt="5842.png" class="jsResizeImage"> Rapid (Buff)</strong>
                                                        <ul>
                                                            <li>
                                                                <strong><span style="color:#008000;">Added:</span></strong><span style="color:#FF8C00;"><strong></strong> Water energies increases by (Player Level * 3 )</span>
                                                            </li>
                                                        </ul>
                                                    </li>
                                                </ul>
                                                <p><br></p>
                                            </div>
                                        </div>
                                        <div class="spoilerBox jsSpoilerBox">
                                            <div class="jsOnly spoilerBoxHeader">
                                                <img src="/assets/img/items/32120.png "> <a class="news-button small jsSpoilerToggle" data-has-custom-label="true"> • MAGICIAN</a>
                                            </div>
                                            <div class="spoilerBoxContent" style="display: none;">
                                                <p><strong><span style="font-size: 14pt;"><img src="" alt="unknown.png" class="jsResizeImage"> Red Magician</span></strong><br></p>
                                                <ul>
                                                    <li>
                                                        <strong><img src="" width="20" alt="unknown.png" class="jsResizeImage"> Mana Transfusion (Buff)</strong>
                                                        <ul>
                                                            <li>
                                                                <span style="color:#FF8C00;">Converts</span> <strong></strong><span style="color:rgb(255,0,0);"><del>40%</del></span><span style="color:rgb(255,140,0);"><strong></strong> </span>
                                                                <i class="fa fa-long-arrow-right" aria-hidden="true"></i>
                                                                <span style="color:rgb(255,140,0);"> <strong></strong></span><strong><span style="color:#008000;">30%</span></strong><span style="color:rgb(255,140,0);"><strong></strong> of received damage by reducing MP</span>
                                                            </li>
                                                        </ul>
                                                    </li>
                                                    <li>
                                                        <strong><img src="/assets/img/items/5857.png" alt="5857.png" class="jsResizeImage"> Fiery Breath</strong>
                                                        <ul>
                                                            <li>Changed Burn Debuff the same as the other specialist skills</li>
                                                        </ul>
                                                    </li>
                                                </ul>
                                                <p><br></p>
                                            </div>
                                        </div>
                                        <div class="spoilerBox jsSpoilerBox">
                                            <div class="jsOnly spoilerBoxHeader">
                                                <img src="/assets/img/items/32160.png "> <a class="news-button small jsSpoilerToggle" data-has-custom-label="true"> • MARTIAL ARTIST</a>
                                            </div>
                                            <div class="spoilerBoxContent" style="display: none;">
                                                <p><strong><span style="font-size: 14pt;"><img src="" alt="unknown.png" class="jsResizeImage"> Draconic Fist</span></strong>
                                                </p>
                                                <ul>
                                                    <li>
                                                        <strong><img src="" width="20" alt="unknown.png" class="jsResizeImage"> Bestial sprint (Buff)</strong>
                                                        <ul>
                                                            <li><span style="color:#FF8C00;">Dodge increases by (Player Level * 2 )</span></li>
                                                        </ul>
                                                    </li>
                                                    <li>
                                                        <strong><img src="" alt="unknown.png" class="jsResizeImage"> Transformation</strong>
                                                        <ul>
                                                            <li>
                                                                <strong>Cool time:</strong> <span style="color:rgb(255,0,0);"><del>5</del></span> 
                                                                <i class="fa fa-long-arrow-right" aria-hidden="true"></i>
                                                                 <strong><span style="color:#008000;">12 seconds</span></strong>
                                                            </li>
                                                            <li>
                                                                <strong><span style="color:#008000;">Added:</span></strong><span style="color:#FF8C00;"><strong></strong> There is a 100% chance of causing [<img src="" alt="unknown.png" class="jsResizeImage"> Evade]</span>
                                                            </li>
                                                        </ul>
                                                    </li>
                                                    <li>
                                                        <strong><img src="" alt="unknown.png" class="jsResizeImage"> Final Flame</strong>
                                                        <ul>
                                                            <li>The skill is now a normal area skill</li>
                                                            <li>
                                                                <img width="20" src="" alt="unknown.png" class="jsResizeImage"> <strong>Final Flame (Debuff)</strong>
                                                                <ul>
                                                                    <li><strong>Debuff Level 2</strong></li>
                                                                </ul>
                                                                <ul>
                                                                    <li>
                                                                        <strong>Duration:</strong> <strong><span style="color:#008000;">2 seconds</span></strong>
                                                                    </li>
                                                                </ul>
                                                                <ul>
                                                                    <li><span style="color:#FF8C00;">No possible attack</span></li>
                                                                </ul>
                                                                <ul>
                                                                    <li><span style="color:#FF8C00;">Movement impossible</span></li>
                                                                </ul>
                                                                <ul>
                                                                    <li><span style="color:#DAA520;"><img width="20" src="" alt="unknown.png" class="jsResizeImage"> Final Flame occurs, in releasing at the probability of 40%</span>
                                                                    </li>
                                                                </ul>
                                                            </li>
                                                            <li>
                                                                <strong><img width="20" src="" alt="unknown.png" class="jsResizeImage"> Final Flame (Debuff)</strong>
                                                                <ul>
                                                                    <li><strong>Debuff Level 3</strong></li>
                                                                </ul>
                                                                <ul>
                                                                    <li>
                                                                        <strong>Duration:</strong> <strong><span style="color:#008000;">2 seconds</span></strong>
                                                                    </li>
                                                                </ul>
                                                                <ul>
                                                                    <li><span style="color:#FF8C00;">No possible attack</span></li>
                                                                </ul>
                                                                <ul>
                                                                    <li><span style="color:#FF8C00;">Movement impossible</span></li>
                                                                </ul>
                                                                <ul>
                                                                    <li><span style="color:#FF8C00;">No dodge possible</span></li>
                                                                </ul>
                                                                <ul>
                                                                    <li><span style="color:#DAA520;"><img width="20" src="" alt="unknown.png" class="jsResizeImage"> Final Flame occurs, in releasing at the probability of 30%</span>
                                                                    </li>
                                                                </ul>
                                                            </li>
                                                            <li>
                                                                <strong><img width="20" src="" alt="unknown.png" class="jsResizeImage"> Final Flame (Debuff)</strong>
                                                                <ul>
                                                                    <li>Debuff Level 4</li>
                                                                    <li>
                                                                        <strong>Duration: </strong><strong><span style="color:#008000;">3 seconds</span></strong>
                                                                    </li>
                                                                    <li><span style="color:#FF8C00;">No possible attack</span></li>
                                                                    <li><span style="color:#FF8C00;">Movement impossible</span></li>
                                                                    <li><span style="color:#FF8C00;">No dodge possible</span></li>
                                                                    <li><span style="color:#FF8C00;">The chance of inflicting a critical hit is increased by 10%</span>
                                                                    </li>
                                                                </ul>
                                                            </li>
                                                        </ul>
                                                    </li>
                                                </ul>
                                                <p><strong><span style="font-size: 14pt;"><br></span></strong></p>
                                            </div>
                                        </div>
                                        <p><br></p>
                                        <p><br></p>
                                        <p><strong><span style="font-size: 18pt;"><img src="/assets/img/bugs.png" alt="bugs.png" class="jsResizeImage"></span></strong>
                                        </p>
                                        <p><strong><br><br></strong></p>
                                        <p>— The maximum 25% is actually 12.5% because it reduces pvp damage by half. Readjusted to 25%
                                            in PvP mode.</p>
                                        <p><br></p>
                                        <p>— Fixed issue in MP consumption reduction effect when using skills, it was being applied
                                            twice in some situations.</p>
                                    </div>
                                </div>
                                <footer class="messageFooter">
                                    <div class="messageFooterNotes">
                                    </div>
                                    <div class="messageFooterGroup">
                                        <ul class="messageFooterButtonsExtra buttonList smallButtons jsMobileNavigationExtra">
                                        </ul>
                                        <ul class="messageFooterButtons buttonList smallButtons jsMobileNavigation">
                                        </ul>
                                    </div>
                                </footer>
                            </div>
                        </article>
                    </li>
                </ul>
            </div>
        </div>
    </div>
</section>